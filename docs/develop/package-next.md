...
下一代Edgeless资源包（下称资源包）本质依旧是一个7z压缩的压缩包，为了便于区分且方便直接使用压缩软件打开，我们使用了`.es.7z`作为拓展名。资源包取代了初代插件包和初代主题包/资源包，使得Edgeless生态更加统一，便于管理。我们也会提供转换工具将初代的包转换为资源包。
...
显然，`package.toml` 取代了初代的外置批处理，成为描述资源包行为的唯一入口文件。使用配置文件而不是包含绝对命令语句的批处理来描述插件包行为有非常多的好处，例如可拓展、可自定义行为、提供更便捷的高级特性等。而选用 [`TOML`](https://toml.io/cn/) 作为配置文件描述语言则是因为 `TOML` 是一种拥有非常好的可读性的高级描述语言，并且能很容易地被转换为 `JSON` 或其他数据结构以便被现代程序处理。当然，我们也会提供相应的 ABNF 语法校验文件、图形化配置生成器和配置检查工具以确保此文件准确地表达了你的意图。

## 基础信息

在 `package.toml` 的开头，我们需要你提供一些这个包的基础信息，大概是这样：
```toml
[package]
# 包名
name = "VSCode"
...
|名称|键|类型|描述|校验|备注|
|-|-|-|-|-|-|
|包名|package.name|String|简名标识资源包内容的字段|需要与文件名中的包名相同；不包含`_`；|<details>如果需要使用`_`，请使用空格或`-`代替；<br/>尽量不要出现代指版本号的数字，请对应的在版本号中体现，例如 `VMware Workstation 16` 就是一个不规范的名称，这会导致用户无法接收大版本更新，因此请将其改为 `VMware Workstation`</details>
|类型|package.type|Enum\<String>|资源包内容类型|`{Software,Driver,Dependency,Theme}`|<details>这是一个 String 类型的枚举类，因此取值只能在校验提供的值中选取，下同</details>
|版本号|package.version|String|资源包内容版本号|`/^\d+.\d+.\d+(.\d+)?$/`；<br/>需要与文件名中的版本号相同|<details>取3位或4位版本号；<br/>如果是软件推荐使用主程序的产品版本，如果是驱动推荐使用设备管理器中标识的驱动程序版本，如果已知上游内容版本号推荐将其缩短至4位以内使用，其他情况推荐令前三位遵守 [Semver](https://semver.org/lang/zh-CN/) 规范</details>
|打包者/作者|package.authors|Array\<String>|打包者与内容作者|打包者（第一个成员）需要与文件名中的打包者相同|<details>统一将打包者作为第一个成员提供，如果打包者同时也是内容作者则只需保留一个成员即可；<br/>可以使用` <github_id>`来提供对应的GitHub ID</details>
|兼容的 Edgeless 版本（可选）|package.compat|Array\<String>|资源包兼容的Edgeless版本|`/^[><]=?\s*\d+.\d+.\d+$/`|<details>允许的前缀：`[">=" , "<=" , ">" , "<"]`；<br/>缺省表示全部兼容</details>
|通过测试的 Edgeless 版本|package.tested|Array\<String>|经打包者测试可用的Edgeless版本|`/^\d+.\d+.\d+$/`|<details>后续可能会引入由Edgeless基础设施提供的版本号</details>
...
为了解决上述问题，我们会使用[工作流](https://zh.wikipedia.org/wiki/%E5%B7%A5%E4%BD%9C%E6%B5%81%E6%8A%80%E6%9C%AF)来描述这个至关重要的流程。在 `package.toml` 中的配置工作流可能是像下面这样：
```toml
  ...
  # 默认链接到桌面，允许用户（事用户！！！）自选位置桌面(Desktop)/开始菜单(StartMenu)/任务栏(TaskBar)
...
看起来可能会比较晦涩，不过这仅仅是为了让你拥有一个大致的概念。我们会在下面详细解释这种写法
...
步骤的键同样由编写者自由决定，推荐使用 [snake_case](https://en.wikipedia.org/wiki/Snake_case) 风格的 `name`。此例中的键为 `copy_config`，访问路径为 `setup_flow.copy_config`
...
:::tip
如果你认为我们提供的官方类型无法很好地描述你编写的基础步骤，请暂时地使用脚本(Script)代替并给我们发issue
:::
### 变量
...
```
#### 用户配置变量
除了使用 `env` 指定不可变的自定义变量，你还可以使用 `uc` 指定可供用户更改的配置变量 (User-controled Config)，例如：
```toml
  [uc.AUTO_RUN]
  name = "开机启动"
...
```
指定了一个布尔型的 `AUTO_RUN` 用户配置变量，此变量的值由用户提供，并通过 `${uc.KEY_NAME}` 调用，例如：
```toml
  [setup_flow.start_vscode]
  name = "Start VSCode"
  type = "Exec"
...
```
:::tip
此处的正则表达式为Perl-style，需要能被[regex crate](https://docs.rs/regex)解析
...
```toml
  ...
  command = "explorer ${Desktop}/Visual Studio Code.lnk"
```
当 `if` 指定的逻辑表达式结果为 `true` 时此步骤才会被执行
