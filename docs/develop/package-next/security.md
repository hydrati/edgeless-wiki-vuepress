...
这意味着在构建完成后5分钟内对资源包进行的修改是被允许的，你可以不通过构建工具再次构建而直接修改资源包内的内容；而5分钟后资源包自锁就会形成，此时不通过构建工具再次构建而直接修改资源包内的内容会导致加载器拒绝加载此资源包。

资源包自锁可以基础地确保 `package.toml` 文件与资源包本体的锁定，使加载器在不依赖于可信凭据中心和 TPM 模块的前提下确保资源包的内容没有在后期受到恶意程序的篡改，同时拥有可以忽略不计的性能开销。
...
## 小心行事！
由于开头提到的两个限制条件影响，Edgeless 资源包在理论上完全无法抵御精心设计的恶意攻击，因此我们并没有引入太多的安全特性。确保安全的最佳实践还是需要你的小心谨慎，谨慎启用某资源包的自动完全加载，保持良好的上网习惯，不要轻易信任不明文件，不要随意关闭安全软件，这样才能使你尽可能远离网络威胁。