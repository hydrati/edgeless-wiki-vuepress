...
- 在根目录中编写*由 `Script` 类型步骤直接调用*的脚本，并在其中使用 `.\_retine\xxx` 调用引用的工具和额外的脚本
- 完成后测试运行构建工具构建资源包，构建工具会自动处理你所使用到的脚本并打包

加载器运行时会遍历 `_retinue` 文件夹中的所有脚本文件(`.cmd` `.bat` `.wcs` `.ini`)，将其中的 `.\_retine\xxx` 引用动态替换为正确的路径，然后再解析工作流，并基于资源根目录直接调用 `_retinue` 文件夹中的脚本。这样可以保证加载时的工作环境与打包前的工作环境基本一致。

:::tip
请注意，只有显式地使用 `.\_retine\` 开头的引用才会被动态替换

如果不希望引用被动态替换，请使用 `_retine\`

如果需要加载器额外对某个文本文件中的 `.\_retine\xxx` 引用进行动态替换，可以在 `Script` 类型的步骤中使用 `fix` 字段指定需要处理的文件
:::
