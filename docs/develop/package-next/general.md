...
如果不希望引用被动态替换，请使用 `_retinue\`，通常需要这么做的情况很罕见

如果需要加载器额外对某个文本文件中的 `.\_retinue\xxx` 引用进行动态替换，可以在 `Script` 类型的步骤中使用 `fix` 字段指定需要处理的文件，例如 `fix = ["./MySoftware/install.cmd","./_retinue/uninstall.bat"]`
:::
