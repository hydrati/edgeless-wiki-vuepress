...
下一代 Edgeless 资源包（下称资源包）本质依旧是一个 7z 压缩的压缩包，为了便于区分且方便直接使用压缩软件打开，我们使用了 `.nep.7z` 作为拓展名，同样也支持 `.nep.7zf` 等带属性的拓展名。资源包取代了初代插件包和初代主题包/资源包，使得 Edgeless 生态更加统一，便于管理。我们也会提供转换工具将初代的包转换为下一代资源包。
...
通常来说你不需要手动建立这个文件夹，构建工具会扫描所有的 `Script` 类型步骤，然后将使用到的脚本文件移动到 `_retinue` 文件夹。在 `_retinue` 文件夹中的脚本的工作目录依旧会是资源根目录，因此你大可直接在根目录调试你的脚本，然后让构建工具自动移动这些脚本。

不过，由于根目录不允许出现规定内容之外的东西，如果你的脚本引用了一些二进制工具(例如调用 [`jj.exe`](https://github.com/tidwall/jj) 读写 json 文件)，那么你需要这样做：

- 将工具和脚本都放置在根目录中进行编写和测试
- 测试结束后新建 `_retinue` 文件夹，将工具移动到 `_retinue` 文件夹内，脚本则依旧留在根目录
- 运行构建工具构建资源包
