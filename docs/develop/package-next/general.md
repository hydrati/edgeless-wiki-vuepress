...
下一代 Edgeless 资源包（下称资源包）本质依旧是一个 7z 压缩的压缩包，为了便于区分且方便直接使用压缩软件打开，我们使用了 `.erp.7z` 作为拓展名，同样也支持 `.erp.7zf` 等带属性的拓展名。资源包取代了初代插件包和初代主题包/资源包，使得 Edgeless 生态更加统一，便于管理。我们也会提供转换工具将初代的包转换为下一代资源包。
...
假设你的资源包文件名为 `MySoftware-runtime_1.0.0_Cno.erp.7z` 且是一个绿色软件，那么你必须将此软件的主程序放置在 `MySoftware-runtime` 目录内；如果这是一个通过安装包静默安装的软件(或真单文件程序<sup>[[1]](#footnote-1)</sup>)，那么你必须将其安装包(或主程序)命名为 `MySoftware-runtime.exe` 放置在根目录下。
...
- 测试结束后新建 `_scripts` 文件夹，将工具移动到 `_scripts` 文件夹内，脚本则依旧存放在根目录
- 运行构建工具构建资源包

在运行时，加载器会将这些二进制工具恢复到资源根目录。不过，为了考虑性能和兼容性，加载器并不能保证你使用的工具不会与其他资源携带的工具产生冲突，因此请谨慎命名这些工具。

:::tip
加载器运行时会动态地重命名你的脚本文件以避免发生脚本覆盖事故，如果需要在脚本中调用另一个脚本，请将*被调用的*脚本拓展名改为 `.bat` (对应 cmd 脚本)或 `.ini` (对应 pecmd 脚本)以将它们视为工具，请同样谨慎命名这些工具
:::
